...
class Config:
    """微调框架的配置类"""
    model_name: str = "./models/deepseek-moe-16b-chat"        # 模型名称 (deepseek-moe-16b-chat) "./models/smallthinker-moe-4b-thinker"
    task_type: str = "causal_lm"                                    # 任务类型
    device: str = "cuda" if torch.cuda.is_available() else "cpu"    # 设备
    
    # model args
    use_lora: bool = True                                           # 是否使用 LoRA (True)
    ...
    dataset_path: str = "./datasets-hauser-full" # "./datasets-hauser4train"
    ...
    save_total_limit: int = 10                                      # 限制总的checkpoints数目减少内存消耗
    gradient_checkpointing: bool = True
    label_names: list[str] = field(default_factory=lambda: ["labels"])
    eval_strategy: str = "steps"
    eval_steps: int = 1000
    lr_scheduler_type: str = "reduce_lr_on_plateau"
    lr_scheduler_kwargs: dict = field(default_factory=lambda: {"factor": 0.95, "patience": 5, "threshold": 1e-3, "min_lr": 1e-5})
    ...
    num_active_experts: int = 4                                     # 模型每层moe激活的experts数量
    chat_mode: bool = False                                          # True的时候直接与模型进行问答，False则正常批量推理
