...
class MoEGate(nn.Module):
    def __init__(self, config):
        ...
        self.consecutive_expert_loss_weight = config.consecutive_expert_loss_weight
        self.consecutive_expert_loss_type = config.consecutive_expert_loss_type
    ...
    def forward(self, hidden_states):
        ...
        if self.training and self.consecutive_expert_loss_weight > 0.0:
            ...
            if self.consecutive_expert_loss_type == 'l2':
                ...
                if new_aux_loss.isnan().any():
                    raise ValueError(f"JS Loss include NaN {new_aux_loss}")
            else:
                raise ValueError(f"Unsupported consecutive_expert_loss_type: {self.consecutive_expert_loss_type}")
            if total_loss is not None:
                total_loss = total_loss + self.consecutive_expert_loss_weight * new_aux_loss
            else:
                total_loss = self.consecutive_expert_loss_weight * new_aux_loss
        # return topk_idx, topk_weight, aux_loss
        return topk_idx, topk_weight, total_loss, scores
